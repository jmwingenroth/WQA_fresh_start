...
crp_files <- list.files(crp_enroll_dir, pattern = "crp_[0-9]*.*tif", full.names = TRUE)
fi_files <- list.files(fi_dir, pattern = "fi_[0-9]*.*tif", full.names = TRUE)
...
if (!all(crp_fips == fi_fips)) stop("State(s) missing CRP enrollment or fertilizer intensity rasters")
...
# Extract FI x CRP enrollment raster data to HUC-12 polygons
fi_huc12 <- list()
    ...
    if(raster_params$year[i] == 0) {
        fi_huc12[[i]] <- huc_12 %>%
            ...
            exact_extract(
                ...
                fun = "sum",
                append_cols = c("huc12", "tohuc", "name", "areasqkm"),
